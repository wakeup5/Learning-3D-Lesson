...
void cScene_03::Init()
{
	...
	this->spotlight.Theta = ONE_RAD * 40.0f;	//안쪽원뿔각 10도 
	this->spotlight.Phi = ONE_RAD * 45.0f;		//바깥쪽원 뿔각 20도
	this->spotlight.Falloff = 1.0f;			//안쪽원뿔에서 바깥쪽원뿔까지 감쇠되는 수치
	this->spotlight.Attenuation0   = 1.0f;
	this->spotlight.Attenuation1   = 1.0f;
	this->spotlight.Attenuation2   = 1.0f;
...
void cScene_03::Update( float timeDelta )
{
	...
	//허벅지는 항상 Target 을 본다.
	//this->BoneMap["RightFoot"]->LookPosition( this->Target.GetWorldPosition() );
