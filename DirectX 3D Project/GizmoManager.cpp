...
namespace NS_ROOT
{
	namespace NS_DIRECTX
	{
		...
		HRESULT GizmoManager::Setup(LPDIRECT3DDEVICE9 device)
		{
			m_pDevice = device;
		...
		//선을 그리는 함수
		void GizmoManager::Line(const D3DXVECTOR3& startPos, const D3DXVECTOR3& endPos, DWORD color) const
		{
			//이전의 월드 행렬을 기억하자.
			...
			vertices[0].p = startPos;
			vertices[0].c = color;
			vertices[1].p = endPos;
			vertices[1].c = color;

			//그린다.
			m_pDevice->SetFVF(PC_VERTEX::FVF);
		...
		//원을 그리는 함수
		void GizmoManager::Circle(const D3DXVECTOR3& centerPos, float radius, const D3DXVECTOR3& axis, DWORD color) const
		{
			//이전의 월드 행렬을 기억하자.
			D3DXMATRIXA16 matPrevWorld;
			m_pDevice->GetTransform(D3DTS_WORLD, &matPrevWorld);
			...
			//임시 정점 배열 
			PC_VERTEX vertices[CIRCLEGIZMO_SEGMENTS + 1];
			for (int i = 0; i < CIRCLEGIZMO_SEGMENTS + 1; i++)
			{
				vertices[i].p = m_pCirclePos[i];
				vertices[i].c = color;
			}
		...
		//와이어 구를 그리는 함수
		void GizmoManager::WireSphere(const D3DXVECTOR3& centerPos, float radius, DWORD color) const
		{
		...
		void GizmoManager::AABBBox(const D3DXVECTOR3& minPos, const D3DXVECTOR3& maxPos, DWORD color) const
		{
