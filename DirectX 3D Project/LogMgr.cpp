...
namespace NS_ROOT
{
	namespace NS_UTIL
	{
		...
		//로그창 설정
		HRESULT	LogMgr::Setup(unsigned int nTarget, HWND hParentWnd, const char* pszFileName)
		{
			m_nTarget = nTarget;
		...
		//로그 찍기
		VOID LogMgr::AddLog(char* strmsg, ...)
		{
		...
		//파일로 로그 저장
		VOID LogMgr::LogFile(char* strmsg, ...)
		{
			//현시간을 얻어온다.
			SYSTEMTIME		st;
			GetLocalTime(&st);
		...
		//로그 윈도우생성
		VOID LogMgr::CreateLogWindow(HWND hWnd)
		{
			//
			// 로그 윈도우 생성
			//
			...
			wc.lpfnWndProc = (WNDPROC)LogMgr::WndLogProc;
			wc.cbClsExtra = 0;
			wc.cbWndExtra = 0;
		...
		//로그 윈도우 메세지 프로시져
		LRESULT CALLBACK LogMgr::WndLogProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
		{
