	...
	namespace NS_OBJECTS
	{
		...
		void Transform::LookDirection(const D3DXVECTOR3& dir, float angle)
		{
			...
			float distRadian = acos(
				Util::ClampMinusOnePlusOne(D3DXVec3Dot(worldAxis + AXIS_Z, &dir)));
		...
		void Transform::DefaultControl(float timeDelta)
		{
			...
			if (KEY_MGR->IsOnceDown(VK_RBUTTON)){
				...
				//다시 마우스 위치를 센터로...
				NS_UTIL::SetMousePos(screenCenterX, screenCenterY);
			}
			...
			//우클릭을 할때만 Default Control 을 한다
				...
				*/
				...
				//현재 마우스 위치
				POINT mousePos = NS_UTIL::GetMousePos();
				...
				//앵글값을 min max 범위 안으로
				nowAngleV = NS_UTIL::Clamp(nowAngleV, minAngleV, maxAngleV);
		...
		void Transform::ShakeUpdate(float timeDelta)
		{
			//위치에 대한 흔들림 파워가 있다면..
			if (this->ShakePosPower > 0.0f && this->ShakePosFlag != 0)
			{
				...
				if (this->ShakePosFlag & SHAKE_X)
					localPos.x = Util::RandomFloatRange(-this->ShakePosPower, this->ShakePosPower);
				if (this->ShakePosFlag & SHAKE_Y)
					localPos.y = Util::RandomFloatRange(-this->ShakePosPower, this->ShakePosPower);
			...
			if (this->ShakeRotPower > 0.0f && this->ShakeRotFlag != 0)
			{
				...
				if (this->ShakeRotFlag & SHAKE_X)
					localRot.x = Util::RandomFloatRange(-this->ShakeRotPower, this->ShakeRotPower);
				if (this->ShakeRotFlag & SHAKE_Y)
					localRot.y = Util::RandomFloatRange(-this->ShakeRotPower, this->ShakeRotPower);
				if (this->ShakeRotFlag & SHAKE_Z)
					localRot.z = Util::RandomFloatRange(-this->ShakeRotPower, this->ShakeRotPower);
